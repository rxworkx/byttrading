...
// ISO 3166-1 alpha-2 code, common name, and calling code. Used for both the
// country select and the phone number's country-code select, on signup and
// on profile/admin editing, so the two stay in sync.
...
// Phone numbers are stored as one string, "<dialCode> <number>" (e.g.
// "+1 5551234567"), so the country-code select and the number field can
// round-trip through a single column without a schema change.
export function parsePhone(phone: string | null | undefined) {
  if (!phone) return { dialCode: "", number: "" };
  const match = phone.match(/^(\+\d+)\s*(.*)$/);
  if (!match) return { dialCode: "", number: phone };
  return { dialCode: match[1], number: match[2] };
}

export function formatPhone(dialCode: string, number: string) {
